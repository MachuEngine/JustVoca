...
class ChatRequest(BaseModel):
    ...
    # 👇 [추가] 이전 대화 내역을 받을 리스트 (role과 content를 가진 딕셔너리 리스트)
...
async def chat_with_ai(request: ChatRequest):
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo", # 예산이 된다면 gpt-4o-mini 추천 (더 똑똑하고 쌈)
            messages=messages,
