...
import { ChevronLeft, Star, BookOpen } from 'lucide-react';

// [수정] 6단계 전체 레벨 정의 (잠금 속성 삭제)
const LEVELS = [
  { 
    id: "초급1", 
    title: "초급 1", 
      ...
      <main className="flex-1 p-6 overflow-y-auto pb-10">
        <div className="mb-6">
          <h2 className="text-2xl font-black text-gray-900 mb-2 leading-tight">
            오늘 학습할<br/>단계를 선택해주세요
          </h2>
          <p className="text-gray-500 font-medium text-sm">
            하루 10개씩, 꾸준함이 실력이 됩니다! 🔥
          </p>
        </div>
        ...
        <div className="space-y-4">
          {LEVELS.map((lvl) => (
            <Link 
              key={lvl.id} 
              href={`/study?level=${lvl.id}`} // 잠금 없이 바로 이동
              className="block group"
            >
              <div className={`
                p-6 rounded-3xl border-2 transition-all duration-200 
                ${lvl.color} bg-opacity-60 hover:bg-opacity-100 hover:shadow-md hover:scale-[1.02] active:scale-[0.98]
              `}>
