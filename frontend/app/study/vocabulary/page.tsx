...
export default function VocabularyStudyPage() {
  ...
  // 2. 데이터 로드 (URL 파라미터 및 로컬 스토리지 연동)
  useEffect(() => {
    async function fetchData() {
      const level = searchParams.get('level') || '초급1';
      const storedUserId = localStorage.getItem('userId') || "학습자"; 

      try {
        setLoading(true);
        const words = await getWords(level, storedUserId); 
        ...
        if (words && words.length > 0) {
          const mappedData = words.map((w: any) => ({
            ...
            audioKey: w.audio_path || "" // 오디오 키가 없을 경우 빈 문자열 처리
          }));
  ...
  const playLocalAudio = (type: 'voca' | 'example', e: React.MouseEvent) => {
    ...
    // [에러 방지] audioKey가 없으면 재생을 시도하지 않음
    if (!currentWord.audioKey) {
      console.warn("오디오 파일 키가 없습니다.");
      return;
    }
              ...
              <div className="flex flex-col items-center w-full animate-in fade-in duration-300">
                ...
                <button 
                  ...
                  className={`flex items-center gap-3 px-10 py-5 text-white font-black rounded-2xl shadow-lg transition-all 
                    ${!currentWord?.audioKey ? 'bg-gray-400 opacity-50 cursor-not-allowed' : 'bg-gray-900 active:scale-95'}`}
                >
                ...
                <div className="grid grid-cols-1 w-full gap-4 px-2">
                  <button 
                    ...
                    className={`w-full h-16 font-black rounded-2xl flex items-center justify-center gap-3 shadow-sm 
                      ${!currentWord?.audioKey ? 'bg-gray-100 text-gray-400' : 'bg-blue-50 text-blue-600 active:bg-blue-100'}`}
                  >
                    <Volume2 size={24} /><span>문장 듣기</span>
                  </button>
          ...
          <div className="absolute inset-0 z-50 animate-in fade-in duration-300 overflow-hidden">
            ...
            <div className="absolute inset-x-0 bottom-0 top-20 bg-white rounded-t-[3rem] shadow-2xl animate-in slide-in-from-bottom duration-500 ease-out flex flex-col">
                  ...
                  {words.map((wordObj: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden transition-all duration-300">
                        <div onClick={() => toggleWordDetail(idx)} className={`p-5 flex items-center justify-between cursor-pointer active:bg-gray-50 ${isExpanded ? 'bg-gray-50/50' : ''}`}>
                          <span className="text-lg font-black text-gray-800">{wordObj.text}</span>
                          <span className={`text-sm font-black px-3 py-1 rounded-full border ${colorClass}`}>{score}점</span>
                        </div>
                        {isExpanded && (
                          <div className="px-5 pb-5 pt-1 bg-gray-50/30 animate-in slide-in-from-top-2">
                            <div className="flex flex-wrap gap-2 mt-2">
                              {wordObj.syll.map((syll: any, sIdx: number) => (
                                <div key={sIdx} className="flex-1 min-w-[70px] bg-white border border-gray-100 rounded-2xl p-3 flex flex-col items-center">
                                  <span className="text-sm font-bold text-gray-600 mb-1">{syll.text}</span>
                                  <span className={`text-xs font-black ${syll.score >= 80 ? 'text-green-500' : 'text-orange-500'}`}>{Math.round(syll.score)}</span>
                                </div>
                              ))}
                            </div>
                          </div>
              ...
              <div className="p-6 pt-0 bg-white grid grid-cols-2 gap-4">
                <button onClick={() => { setShowResultOverlay(false); setRecordingStatus('idle'); }} className="h-16 bg-gray-100 text-gray-600 font-black rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-all"><RotateCcw size={18} /> 다시 녹음</button>
                <button onClick={() => moveToWord('next')} className="h-16 bg-gray-900 text-white font-black rounded-2xl flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">다음 단어 <ArrowRight size={18} /></button>
              </div>
            </div>
          </div>
        )}
